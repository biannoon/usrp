...
<script type="text/javascript">
    $(function () {
        ...
        $('#tree_sysOrgInfo').tree({
            ...
            onLoadSuccess:function (node,data) {
                ...
                //回显勾选指定的值（目前用于单选）
                if (value != '' && value != null){
                    var id = exchangeOrgId('<%=basePath%>',value);
                    var tar = $(this).tree('find', id);
                    //$(this).tree('expandAll');
                    $(this).tree('check',tar.target);
                }
            },
     ...
     *描述：将机构编码转换为机构id
     * */
    function exchangeOrgId(url,orgCd){
        var result = '';
        $.ajax({
            async:false,
            url:url+'sysGroup/exchangeOrgCd',
            type:'post',
            dataType:'json',
            data:{'orgCd':orgCd},
        ...
        if (nodes == null){
            ...
            $.each(nodes,function (index,node) {
                ...
                if (_fieldId != '' && _fieldId != null){
                    $('#'+_fieldId).textbox('setValue',data.attributes.org_cd);
                }
            })
            $('#'+winId).window('close');
        }

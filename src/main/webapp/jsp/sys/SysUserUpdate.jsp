...
<form id="detail_form"  method="post">
    <table width="100%" style="text-align: center" id = "tab1" cellpadding="5">
        <tr>
            ...
            <td align="left">
                <input class="easyui-validatebox" id="userId_insert" name="userId" data-options="required:true,validType:''">
            </td>
        ...
        <tr>
            ...
            <td align="left">
                ...
                <input class="easyui-textbox" id="blngtoOrgNo_hid" name="blngtoOrgNo" type="hidden" value="<%=sysUser.getBlngtoOrgNo()==null?"":sysUser.getBlngtoOrgNo()%>"/>
            </td>
...
<script type="text/javascript">
    $(function(){
        //-赋值
        initTextboxByUser('userId_insert','<%=sysUser.getUserId()==null?"":sysUser.getUserId()%>',6,true,'200','20');
        initTextboxByUser('userNm_insert','<%=sysUser.getUserNm()==null?"":sysUser.getUserNm()%>',60,false,'200','20');
        initTextboxByUser('crtfNo_insert','<%=sysUser.getCrtfNo()==null?"":sysUser.getCrtfNo()%>',20,false,'200','20');
        initTextboxByUser('telNo_insert','<%=sysUser.getTelNo()==null?"":sysUser.getTelNo()%>',20,false,'200','20');
        initTextboxByUser('addr_insert','<%=sysUser.getAddr()==null?"":sysUser.getAddr()%>',200,false,'200','20');
        initTextboxByUser('blngtoOrgNo_insert','<%=EhcacheUtil.getSysOrgInfoByOrgCd(sysUser.getBlngtoOrgNo()).getName()%>',0,false,'200','20');
        initComboboxByUser('genderCd_insert','CD0006','<%=sysUser.getGenderCd()%>',false,'200','20');
        initComboboxByUser('stus_insert','SYS05','<%=sysUser.getStus()%>',false,'200','20');
        initDateBoxByUser('brthdy_insert','<%=sysUser.getBrthdy()==null?"":sysUser.getBrthdy()%>','200','20');
        initPasswordBoxByUser('pwd_insert','<%=sysUser.getPwd()==null?"":sysUser.getPwd()%>','200','20');
    ...
    function save() {
        ...
        //取消禁用
        initTextboxByUser('userId_insert','<%=sysUser.getUserId()%>',6,false,'200','20');
        ajaxSubmit("post",basePath+"sysUser/update",$("#detail_form").serialize(),"json");
    }
